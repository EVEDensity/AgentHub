...
Phase A provider smoke 已支持 `deepseek-v4-flash`/`deepseek-v4-pro` 矩阵、tool-call `call_id`/JSON 参数校验和脱敏 JSON artifact 输出；本地无 Secret 时按设计返回 `SKIP`。Phase B 已通过 pseudo-TTY、无 TTY 和窄宽度自动化测试；物理终端行为仍需 CI runner 录制确认。
