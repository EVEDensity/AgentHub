...
│  Receipts 检索：Mission/Evidence 上的 FTS/关键词视图 + 证据回溯        ⚪               │
└──────┬──────────────────────────┬──────────────────────────────┬──────────────────────┘
       ▼                          ▼                              ▼
┌───────────────┐      ┌────────────────────┐      ┌──────────────────────────────┐
...
| Receipts 检索 | 跨会话任务检索带证据回溯 | ⚪ | 本方案 §8 |
...
| `mission.created` | 触发路由 | 含 mission_id，回链执行域 |
...
```text
检索入口：
  CLI   agenthub search "上个月修过什么"        ⚪ 规划
  会话  @archivist 查一下上次登录重构的结论      ⚪ 规划（同一检索服务）
        │
        ▼
Receipts 视图（Mission/Evidence 上的 FTS/关键词索引 + 时间过滤）
        │  命中记录：objective、终态、verdict、artifact 引用、时间
        ▼
带证据回答：每条结论附 mission 链接 + VERIFY 结果 + Artifact 摘要
        │
        ▼
回放：agenthub replay <mission_id>（已实现）沿执行检查点复现全过程
```
...
| Receipts | Mission/Evidence 跨会话检索 | FTS/关键词视图 | ⚪ |
| 向量 | embedding 检索 | **默认关闭**，opt-in + 验收标准 | 禁止默认 |

溯源链：**结论 → mission_id → WorkUnit 尝试 → Artifact（digest）→
...
| 优先级 | 工作项 | 依赖 | 一句话验收 |
|---|---|---|---|
| P0 | Receipts 检索切片：Mission/Evidence FTS 视图 + `agenthub search` | 无（纯读路径） | 检索"上个月修过什么"返回带 mission 链接与 VERIFY 结果的条目 |
