...
当前进度：已加入 `scripts/cli_provider_smoke.py` 与无密钥流式契约测试；已在受控环境完成 DeepSeek 文本流 smoke。后续真实矩阵固定使用 `v4-flash`/`v4-pro`，完整 `assistant.delta -> tool -> verification` 仍需 CI 验收。
...
当前进度：CLI 已支持按工具名及 glob 路径的 `/allow`、`/deny`、`/permissions`、`/clear-permissions`；规则以版本化 `.agenthub/permissions.json` 持久化并在会话启动时加载，后端 PermissionManager 的路径/风险规则仍是最终裁决。更丰富的交互式策略编辑和跨设备同步仍待后续迭代。
...
当前进度：已固化 JSONL 的 `schemaVersion: 1` 外层记录，并新增 `.github/workflows/cli-jsonl-smoke.yml`；已加入多 provider 流式/工具调用契约矩阵、`verify_cli_release.py` 发布前置检查、DeepSeek v4-flash/v4-pro 文本与 tool-call nightly，以及手动触发的 Windows/macOS/Linux npm 安装验证。Windows job 覆盖目标版本与上一版本切换，真实 registry、升级和回滚结果仍需 CI 执行记录。
