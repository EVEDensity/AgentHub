...
def test_permission_policy_is_authenticated_and_explains_source(monkeypatch):
    ...
    monkeypatch.setattr(permissions, "afetch_all", lambda *args: [{"id": 1, "agent_id": "u1", "tool_pattern": "shell", "path_pattern": "src/*", "behavior": "deny", "source": "org", "priority": 50, "enabled": 1}])
...
def test_permission_sync_rejects_other_user_scope(monkeypatch):
    ...
    app.include_router(permissions.router, prefix="/api/v1")
    app.dependency_overrides[get_current_user] = lambda: {"id": "u1", "role": "developer"}
    monkeypatch.setattr(permissions, "aexecute", lambda *args: None)
    with TestClient(app) as client:
