...
class StartupMigrationTests(unittest.IsolatedAsyncioTestCase):
    async def test_fresh_database_creates_tables_before_stamping_head(self) -> None:
        ...
        self.assertEqual(connection.current_revision, DECISION_EXPIRY_REVISION)
    ...
    async def test_current_head_is_idempotent(self) -> None:
        connection = FakeConnection(DECISION_EXPIRY_REVISION)
    ...
    async def test_artifact_head_advances_only_evidence_projection(self) -> None:
        ...
        self.assertNotIn(artifact_table, statements)
        backfill = next(
            statement
            for statement in EVIDENCE_PROJECTION_UPGRADE
