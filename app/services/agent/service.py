...
class AgentService:
    ...
    def choose_models(models: list[dict]) -> list[dict]:
        ranked = sorted(models, key=AgentService._score, reverse=True)
        return ranked
    ...
    async def call_agent(session_id: str, content: str, user_id: str) -> dict:
        ...
        for model in models:
            ...
            try:
                ...
                AgentService._update_runtime(model, True, (time.perf_counter() - started) * 1000)
                break
            except Exception as exc:
                AgentService._update_runtime(model, False, (time.perf_counter() - started) * 1000)
    ...
    @staticmethod
    def normalize_agent_output(agent_id: str, output: str, input_text: str) -> str:
        if agent_id == "CodeGen":
            return output
        return output.strip()
