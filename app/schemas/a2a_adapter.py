...
def _normalize_source_agent_url(value: str) -> str:
    ...
    if parsed.username is not None or parsed.password is not None:
        raise ValueError("sourceAgentUrl cannot contain user information")
    if parsed.fragment:
        raise ValueError("sourceAgentUrl cannot contain a fragment")
    if parsed.query or parsed.path not in {"", "/"}:
        raise ValueError("sourceAgentUrl must contain only an HTTP(S) origin")
    try:
        port = parsed.port
    except ValueError as exc:
        raise ValueError("sourceAgentUrl contains an invalid port") from exc
...
class A2AInboundTaskCreateRequest(BaseModel):
    model_config = ConfigDict(
        alias_generator=_to_camel,
        extra="forbid",
        populate_by_name=True,
    ...
    objective: Annotated[str, Field(min_length=1, max_length=10000)]
    source_agent_url: Annotated[str, Field(min_length=1, max_length=2048)]
    required_capabilities: list[
        Annotated[str, Field(min_length=1, max_length=255)]
    ] = Field(default_factory=list)
    time_seconds: Annotated[int, Field(ge=1, le=86400)] = 3600
...
class A2AInboundTaskCancelRequest(A2ATaskCancelRequest):
    source_agent_url: Annotated[str, Field(min_length=1, max_length=2048)]
