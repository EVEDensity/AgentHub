...
// ── Topology Canvas (force-directed graph visualization) ─────────────
...
function TopologyCanvas({
  ...
  useEffect(() => {
    ...
    // Initialize positions for each node (random but spread)
    interface SimNode {
      id: string;
      x: number;
      y: number;
    ...
    const simNodes: SimNode[] = nodes.map((n) => ({
      ...
      status: n.status,
      label: n.label || n.id,
      radius: n.type === 'agent' ? 18 : n.type === 'task' ? 12 : 10,
    }));
    ...
    const COLORS: Record<string, string> = {
      idle: '#22c55e', busy: '#f59e0b', overloaded: '#ef4444', offline: '#9ca3af',
      pending: '#6b7280', running: '#f59e0b', completed: '#22c55e', failed: '#ef4444',
      created: '#8b5cf6', destroyed: '#9ca3af',
    };
    ...
    function draw() {
      ...
      edges.forEach((e) => {
        ...
        ctx!.strokeStyle = e.status === 'completed' ? 'rgba(34,197,94,0.3)' :
          e.status === 'failed' ? 'rgba(239,68,68,0.3)' :
      ...
      // Draw nodes
      simNodes.forEach((n) => {
        const color = COLORS[n.status] || '#9ca3af';
        ...
        // Type icon
        ctx!.fillStyle = '#fff';
        ctx!.font = `${n.radius}px system-ui`;
        ctx!.textAlign = 'center';
        ctx!.textBaseline = 'middle';
        const icon = n.type === 'agent' ? '🤖' : n.type === 'task' ? '📋' : '🧬';
        ...
        ctx!.textAlign = 'center';
        const label = n.label.length > 15 ? n.label.slice(0, 15) + '...' : n.label;
    ...
    function simulate() {
      ...
      for (let i = 0; i < simNodes.length; i++) {
        ...
        // Center gravity
        a.vx += (W / 2 - a.x) * centerGravity;
        a.vy += (H / 2 - a.y) * centerGravity;
  ...
  return (
    <div className="card p-4">
      <canvas
        ref={canvasRef}
        className="w-full rounded-lg bg-gray-50/50"
        style={{ height: '400px' }}
      />
      <div className="flex flex-wrap gap-3 mt-3 text-xs text-gray-400">
        <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-green-500" /> idle/完成</span>
        <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-amber-500" /> busy/运行中</span>
        <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-red-500" /> 过载/失败</span>
        <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-gray-400" /> 离线</span>
        <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-purple-500" /> 新创建</span>
      </div>
    </div>
  );
}
