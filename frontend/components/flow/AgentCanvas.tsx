...
export default function AgentCanvas(props?:{
 ...
 function add(b:(typeof lib)[number]){const i=doc.nodes.length+1,n:N={id:`${b.type}-${Date.now()}`,type:b.type,title:b.title==='Agent'?`Agent ${i}`:b.title,desc:b.desc,x:190+i*30,y:140+i*24,w:W,h:H,color:b.color,z:i,agent:embedded&&b.type==='agent'?props?.agents?.[0]?.agentId:undefined,layer:b.type==='start'?'meta':b.type==='end'?'micro':'domain'};commit({...doc,nodes:[...doc.nodes,n]});setSel(n.id)}
 ...
 return <div className={`flex ${embedded?'h-full':'h-screen'} bg-[#F7F7F4] text-warm-800`}>
  <aside className="flex w-[280px] flex-col border-r border-warm-150 bg-white">
   <div className="border-b border-warm-150 p-5">
    <div className="text-h2">Agent Flow</div>
    <p className="mt-1 text-caption text-warm-500">低代码 Agent 连接画布</p>
    {!embedded&&<a className="btn-ghost mt-3 block px-0 text-primary-500" href="/">返回会话</a>}
    {embedded&&<a className="btn-ghost mt-3 block px-0 text-primary-500" href="/canvas" target="_blank">全屏画布</a>}
   </div>
   <div className="p-4">
    <div className="mb-3 text-caption font-semibold uppercase tracking-wide text-warm-500">Blocks</div>
    <div className="space-y-2">{lib.map(b=><button key={b.type} className="w-full rounded-xl border border-warm-150 bg-white p-3 text-left transition hover:border-primary-200 hover:shadow-card" onClick={()=>add(b)}><span className="mr-3 inline-block h-3 w-3 rounded-full" style={{background:b.color}}/><span className="font-medium">{b.title}</span><div className="mt-1 text-caption text-warm-500">{b.desc}</div></button>)}</div>
   </div>
   <div className="mt-auto border-t border-warm-150 p-4 text-caption text-warm-500">Ctrl/⌘+Z 撤销，Shift+Z 重做，Ctrl/⌘+D 复制，Esc 退出连线，Delete 删除。</div>
  </aside>
  <main className="flex min-w-0 flex-1 flex-col">
   <header className="flex h-16 items-center justify-between border-b border-warm-150 bg-white px-5">
    <div>
     {embedded?<div className="flex items-center gap-3">
      <input className="input-field w-40 text-sm" placeholder="工作流名称" value={wfName} onChange={e=>setWfName(e.target.value)}/>
      <input className="input-field w-48 text-sm" placeholder="描述" value={wfDesc} onChange={e=>setWfDesc(e.target.value)}/>
      <input className="input-field w-40 text-sm" placeholder="触发关键词（逗号分隔）" value={wfKeys} onChange={e=>setWfKeys(e.target.value)}/>
     </div>:<><div className="text-h3">Workflow Canvas</div><div className="text-caption text-warm-500">缩放 {Math.round(doc.viewport.scale*100)}% · 节点 {doc.nodes.length} · 连线 {doc.edges.length}</div></>}
    </div>
    <div className="flex items-center gap-2">
     {msg&&<span className="rounded-lg bg-primary-50 px-3 py-2 text-caption text-primary-600">{msg}</span>}
   ...
   <div className="flex min-h-0 flex-1">
    <section ref={wrap} className="relative flex-1 overflow-hidden bg-[#FAFAF8]" style={{backgroundImage:'linear-gradient(#ECEBE6 1px,transparent 1px),linear-gradient(90deg,#ECEBE6 1px,transparent 1px),linear-gradient(#F4F3EE 1px,transparent 1px),linear-gradient(90deg,#F4F3EE 1px,transparent 1px)',backgroundSize:'120px 120px,120px 120px,24px 24px,24px 24px'}}>
     <Stage ref={stage} width={sz.width} height={sz.height} x={doc.viewport.x} y={doc.viewport.y} scaleX={doc.viewport.scale} scaleY={doc.viewport.scale} draggable dragDistance={3}
      ...
      <Layer listening={false}>{grid.v.map(x=><Line key={`v${x}`} points={[x,grid.minY,x,grid.maxY]} stroke={x%120===0?'#DDDCD4':'#ECEBE6'} strokeWidth={x%120===0?1.2:.8}/>)}{grid.h.map(y=><Line key={`h${y}`} points={[grid.minX,y,grid.maxX,y]} stroke={y%120===0?'#DDDCD4':'#ECEBE6'} strokeWidth={y%120===0?1.2:.8}/>)}</Layer>
      <Layer>
       {doc.edges.map(e=>{const a=map.get(e.from),b=map.get(e.to);if(!a||!b)return null;const p=edgePath(a,b),act=edgeHover===e.id||sel===e.from||sel===e.to;return <Group key={e.id} onMouseEnter={()=>setEdgeHover(e.id)} onMouseLeave={()=>setEdgeHover('')}><Arrow points={p.points} bezier stroke={act?'#3D5AFE':'#9A9890'} fill={act?'#3D5AFE':'#9A9890'} strokeWidth={act?3.6:2.4} pointerLength={12} pointerWidth={12} lineCap="round" lineJoin="round" shadowColor={act?'#4F6CF7':'#000'} shadowBlur={act?12:4} shadowOpacity={act?.22:.08}/><Circle x={p.start.x} y={p.start.y} radius={4.8} fill={a.color} stroke="#FFF" strokeWidth={2}/>{e.label&&<Group x={p.label.x-24} y={p.label.y-15}><Rect width={48} height={24} cornerRadius={12} fill="#FFF" stroke={act?'#B8C3FF':'#E1E0DA'} shadowBlur={8} shadowOpacity={.08}/><Text x={6} y={6} width={36} align="center" text={e.label} fontSize={11} fill={act?'#3D5AFE':'#6D6B65'}/></Group>}</Group>})}
       {sorted.map(n=><Group key={n.id} ref={r=>{refs.current[n.id]=r}} x={n.x} y={n.y} draggable dragDistance={3} scaleX={pressed===n.id?.985:1} scaleY={pressed===n.id?.985:1} onMouseDown={()=>setPressed(n.id)} onMouseUp={()=>setPressed('')} onClick={()=>{setSel(n.id);link(n.id)}} onTap={()=>setSel(n.id)} onDragStart={e=>{e.cancelBubble=true;setSel(n.id);setPressed(n.id)}} onDragEnd={e=>{e.cancelBubble=true;setPressed('');patch(n.id,{x:e.target.x(),y:e.target.y()})}} onTransformEnd={e=>{const t=e.target,sx=t.scaleX(),sy=t.scaleY();t.scaleX(1);t.scaleY(1);patch(n.id,{x:t.x(),y:t.y(),w:Math.max(180,n.w*sx),h:Math.max(86,n.h*sy)})}}>
        <Rect width={n.w} height={n.h} cornerRadius={20} fill="#FFF" stroke={from===n.id?'#111827':sel===n.id?n.color:'#E1E0DA'} strokeWidth={from===n.id?3.4:sel===n.id?3:1.3} shadowColor={sel===n.id||from===n.id?n.color:'#111827'} shadowBlur={sel===n.id||from===n.id?22:16} shadowOpacity={sel===n.id||from===n.id?.18:.08} shadowOffsetY={sel===n.id||from===n.id?10:8}/>
        <Rect width={5} height={n.h-28} x={14} y={14} cornerRadius={4} fill={n.color}/>
        <Circle x={31} y={30} radius={7} fill={n.color} opacity={.18}/>
        <Text x={46} y={22} text={n.title} fontSize={17} fontStyle="bold" fill="#24231F" width={n.w-84}/>
        <Text x={34} y={52} text={n.desc} fontSize={13} fill="#6D6B65" width={n.w-58} lineHeight={1.25}/>
        <Group x={34} y={n.h-32}><Rect width={86} height={20} cornerRadius={10} fill={`${n.color}1A`}/><Text x={10} y={5} text={n.type.toUpperCase()} fontSize={10} fill={n.color} fontStyle="bold"/></Group>
        <Circle x={n.w-22} y={n.h/2} radius={8} fill={from===n.id?'#111827':n.color} stroke="#FFF" strokeWidth={2}/>
        <Circle x={n.w-22} y={n.h/2} radius={13} stroke={from===n.id?'#111827':n.color} strokeWidth={1.5} opacity={sel===n.id||from===n.id?.32:.12}/>
       </Group>)}
       <Transformer ref={tr} rotateEnabled={false} boundBoxFunc={(oldBox,newBox)=>newBox.width<180||newBox.height<86?oldBox:newBox}/>
      </Layer>
     </Stage>
     <div className="pointer-events-none absolute bottom-5 left-5 rounded-xl border border-warm-150 bg-white/90 px-4 py-3 text-caption text-warm-500 shadow-card">拖动画布平移，滚轮以鼠标位置为中心缩放；点击节点后再点目标节点完成业务流程关系。当前连线源：{from||'无'}。</div>
    </section>
    <aside className="w-80 border-l border-warm-150 bg-white p-5">
     <div className="mb-4 flex items-center justify-between">
      <div><div className="text-h3">属性面板</div><div className="text-caption text-warm-500">Selection Inspector</div></div>
      <button className="btn-ghost px-2 text-danger-500" onClick={del}>删除</button>
     </div>
     {sn?<div className="space-y-4">
      <label className="block text-h4">节点名称<input className="input-field mt-2" value={sn.title} onChange={e=>patch(sn.id,{title:e.target.value})}/></label>
      <label className="block text-h4">描述<textarea className="input-field mt-2 min-h-[88px] resize-none" value={sn.desc} onChange={e=>patch(sn.id,{desc:e.target.value})}/></label>
      <input className="h-10 w-full rounded-lg border border-warm-150" type="color" value={sn.color} onChange={e=>patch(sn.id,{color:e.target.value})}/>
      {embedded&&sn.type==='agent'&&<>
       <label className="block text-h4">Agent<select className="input-field mt-2 text-sm" value={sn.agent||''} onChange={e=>patch(sn.id,{agent:e.target.value})}>{agentOpts.map(a=><option key={a.agentId} value={a.agentId}>{a.agentId}</option>)}</select></label>
       <label className="block text-h4">层级<select className="input-field mt-2 text-sm" value={sn.layer||'domain'} onChange={e=>patch(sn.id,{layer:e.target.value})}><option value="meta">Layer1 (Meta)</option><option value="domain">Layer2 (Domain)</option><option value="micro">Layer3 (Micro)</option></select></label>
      </>}
      {embedded&&<label className="block text-h4">依赖节点<div className="mt-1 text-caption text-warm-500">{(depMap.get(sn.id)||[]).join(',')||'无（通过连线自动计算）'}</div></label>}
      <div className="grid grid-cols-2 gap-2">
       <Btn onClick={dup}>复制</Btn><Btn onClick={()=>{setFrom(sn.id);fromRef.current=sn.id;setMsg('连线模式：请选择目标节点')}}>连线</Btn>
       <Btn onClick={()=>patch(sn.id,{z:Math.max(...doc.nodes.map(n=>n.z))+1})}>置顶</Btn><Btn onClick={()=>patch(sn.id,{z:Math.min(...doc.nodes.map(n=>n.z))-1})}>置底</Btn>
      </div>
      <div className="rounded-xl bg-warm-50 p-3 text-caption text-warm-600">ID：{sn.id}<br/>位置：{Math.round(sn.x)}, {Math.round(sn.y)}<br/>大小：{Math.round(sn.w)} × {Math.round(sn.h)}</div>
     </div>:<div className="rounded-xl bg-warm-50 p-4 text-sm text-warm-500">请选择一个节点编辑属性。</div>}
     <div className="mt-6 border-t border-warm-150 pt-4">
      <div className="mb-3 text-h4">图层管理</div>
      <div className="max-h-72 space-y-2 overflow-auto pr-1">{[...sorted].reverse().map(n=><button key={n.id} className={`w-full rounded-lg border px-3 py-2 text-left text-sm ${sel===n.id?'border-primary-200 bg-primary-50 text-primary-600':'border-warm-150 bg-white text-warm-600'}`} onClick={()=>setSel(n.id)}>{n.title}</button>)}</div>
     </div>
    </aside>
   </div>
  </main>
