...
export default function AIChatDialog({ open, onClose, addConsoleLog }: AIChatDialogProps): JSX.Element {
  ...
  const handleSend = useCallback(async () => {
    ...
    // Create placeholder message that will be filled by streamed AI response
    const aiMsg = addMessage('assistant', '');
    setIsStreaming(true);

    try {
      // Call real AI backend via Next.js API route (proxies to Go gateway)
      const res = await fetch('/api/chat/send', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          message: trimmed,
          agent_id: selectedModel.includes('Claude') ? 'claude' : undefined,
          session_id: `ai-dialog-${Date.now()}`,
          stream: false,
        }),
      });
      ...
      if (!res.ok) {
        ...
        // Gateway unavailable — gracefully fall back to a helpful notice
        // so the user can still use the dialog UI even when the backend
        // gateway is offline.
        setMessages((prev) =>
          prev.map((m) =>
            m.id === aiMsg.id
              ? {
                  ...m,
                  content:
                    `⚠️ 暂未连接到 AI 后端（HTTP ${res.status}）\n\n` +
                    `你的消息已记录：\n> ${trimmed}\n\n` +
                    `可能原因：\n` +
                    `1. Go Gateway 未启动（默认 http://127.0.0.1:8081）\n` +
                    `2. AI 服务暂时不可用\n` +
                    `3. API Key 未配置\n\n` +
                    `详细错误：${errText.slice(0, 200) || '无'}`,
                }
              : m,
          ),
        );
        addConsoleLog?.('ERR', `AI 请求失败: HTTP ${res.status}`, 'error');
        return;
      }
      ...
      if (replyText) {
        // Stream the reply into the placeholder for a natural typing feel
        simulateStreaming(replyText, aiMsg.id);
        addConsoleLog?.('AI', `响应: ${replyText.slice(0, 60)}${replyText.length > 60 ? '…' : ''}`, 'success');
      } else {
        setMessages((prev) =>
          prev.map((m) =>
            m.id === aiMsg.id
              ? {
                  ...m,
                  content: `✅ 请求已提交（${data.status || 'accepted'}，trace: ${(data.trace_id || '').slice(0, 8) || 'n/a'}）\n\nAI 正在异步处理中，可在主聊天页查看完整进度。`,
                }
              : m,
          ),
        );
        addConsoleLog?.('AI', `异步提交: ${data.trace_id || 'ok'}`, 'info');
      }
    } catch (err) {
      // Network error — show graceful message instead of crashing
          ...
          m.id === aiMsg.id
            ? {
                ...
                content:
                  ...
                  `> ${errMsg}\n\n` +
                  `请检查：\n` +
                  `• Go Gateway 是否运行在 http://127.0.0.1:8081\n` +
                  `• 网络连接是否正常\n` +
                  `• 浏览器控制台是否有 CORS 错误`,
              }
            : m,
      ...
      addConsoleLog?.('ERR', `AI 请求异常: ${errMsg}`, 'error');
    } finally {
      // simulateStreaming will flip isStreaming=false on its own;
      // here we only flip if the request returned without a streamable reply.
      setIsStreaming(false);
    }
  }, [input, isStreaming, addMessage, simulateStreaming, addConsoleLog, selectedModel]);
    ...
    <>
      ...
      <div
        ...
        style={{
          ...
          maxHeight: minimized ? 'auto' : 'min(720px, calc(100vh - 96px))',
          height: minimized ? 'auto' : 'min(720px, calc(100vh - 96px))',
