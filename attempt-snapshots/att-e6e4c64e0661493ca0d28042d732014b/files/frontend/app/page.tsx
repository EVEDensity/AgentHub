...
export default function AgentHubIM(): JSX.Element {
  ...
  // T7: Event timeline panel visibility
  ...
  // ── T7: Session Event SSE stream (realtime timeline) ────────────
  const {
  ...
  // ── Wire session events into session message store ──────────────
  // When the session event stream delivers a new event, surface it as
  ...
  useEffect(() => {
    ...
    const seen = prevSessionEventCountRef.current;
    if (sessionEvents.length <= seen) return;
    const newEvents = sessionEvents.slice(seen);
    prevSessionEventCountRef.current = sessionEvents.length;
    for (const evt of newEvents) {
      // Render event.type as a visible system message (MessageList filters by type)
      const msg: Message = {
        id: `sess-evt-${evt.id}`,
        event: 'mission',
        sessionId: activeSessionId,
        sender: evt.actor.displayName || evt.actor.type,
        content: `[${evt.eventType}] ${typeof evt.payload === 'object' ? JSON.stringify(evt.payload).slice(0, 120) : evt.payload}`,
        type: 'system',
    ...
    <div className="workspace-layout" style={{ '--sidebar-w': `${sidebarW}px`, gridTemplateColumns: gridCols } as React.CSSProperties}>
      ...
      <div className="workspace-main">
        ...
        <div className="workspace-content">
          {/* T1-2b: @archivist 搜索结果卡片 */}
          {archivistInfo && archivistInfo.receiptsCount > 0 && (
            <div className="px-3 pt-3">
              <ArchivistReceiptCard info={archivistInfo} />
            </div>
          )}
          ...
          {/* T7: Realtime Session Event Timeline */}
          <div className="px-3 pb-2">
            ...
            >
              ...
              <span>会话事件流</span>
              <span className="ml-1 rounded bg-warm-100 px-1.5 py-0.5 text-xs text-warm-500">
                {sessionEvents.length}
              </span>
              {sessionEventsState === 'streaming' && (
                <span className="ml-1 inline-flex items-center gap-0.5 text-xs text-emerald-500">
