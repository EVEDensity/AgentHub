...
// ═══════════════════════════════════════════════════════════════════════
// T1-2b + T7: Session Event Stream types (wire contract from /sessions/{id}/events/stream)
// ═══════════════════════════════════════════════════════════════════════

/** Actor reference — who caused the event. */
export interface SessionEventActor {
  type: 'human' | 'service' | 'agent' | 'adapter' | 'runner' | 'verifier';
  id: string;
  displayName: string;
}

/** Session event types emitted by the control plane. */
...
/** Wire payload from GET /api/v1/sessions/{sessionId}/events/stream */
...
/** Archivist receipt (T1-2b) — pre-search result inline in chat_mission response. */
export interface ArchivistReceipt {
  mission_id?: string;
  status?: string;
  verdict?: string;
  objective?: string;
...
/** Session-level receipt bundle returned from chat_mission when @archivist is present. */
export interface ArchivistInfo {
  query: string;
  receiptsCount: number;
  receipts: ArchivistReceipt[];
}

// ═══════════════════════════════════════════════════════════════════════
// Mission Event — wire contract from POST /api/v1/chat/mission SSE stream
// ═══════════════════════════════════════════════════════════════════════

/** A single Mission ledger event streamed over SSE during chat mission execution. */
export interface MissionEvent {
  /** Mission aggregate id */
  missionId?: string;
  /** Alternative id field used by some event types */
  aggregateId?: string;
  /** Domain event type (e.g. 'work_unit.started', 'evidence.recorded') */
  eventType?: string;
  ...
  /** Alternative artifact content address field */
  content_address?: string;
  /** Error/reason text */
  reason?: string;
