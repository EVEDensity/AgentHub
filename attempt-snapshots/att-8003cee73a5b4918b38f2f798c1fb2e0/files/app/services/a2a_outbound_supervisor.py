...
class A2AOutboundSupervisor:
    ...
    async def _run_remote_lifecycle(
        ...
        # Use time.monotonic() for high-resolution timeout management.
        # On Windows, asyncio.timeout fires with ~16ms granularity which
        # can drop polls even when remaining budget is clearly positive.
        # Deadline starts *before* send to match the old asyncio.timeout
        # semantics where send latency also erodes the polling window.
        ...
        try:
            ...
            while snapshot.state in {
                ...
                # Sleep at most poll_interval, but never past the deadline.
        ...
        except _RemotePollingTimeout:
            raise
        # Any other exception (including TimeoutError from transport,
        # CancelledError, etc.) propagates to the caller unchanged —
        # the supervise() wrapper classifies it based on type.
