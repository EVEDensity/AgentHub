...
class _ArtifactSetV1Parameters(_Projection):
    minimum_artifacts: Annotated[int, Field(ge=1, le=200)]
    required_artifact_kinds: tuple[ArtifactKind, ...] = ()

    @model_validator(mode="after")
    def validate_unique_kinds(self) -> _ArtifactSetV1Parameters:
...
class _ArtifactSetV2Parameters(_ArtifactSetV1Parameters):
    optional_artifact_kinds: tuple[ArtifactKind, ...] = ()
    max_artifact_size_bytes: Annotated[int | None, Field(ge=0, le=100_000_000_000)] = None


class _TestRunParameters(_Projection):
    minimum_test_results: Annotated[int, Field(ge=1, le=200)]
    minimum_pass_rate: Annotated[float, Field(ge=0.0, le=1.0)]


class _BuildArtifactParameters(_Projection):
    minimum_build_artifacts: Annotated[int, Field(ge=1, le=200)]
    max_artifact_size_bytes: Annotated[int, Field(ge=1, le=100_000_000_000)]


class _SecurityScanParameters(_Projection):
    minimum_scan_reports: Annotated[int, Field(ge=1, le=200)]
    max_severity: Annotated[int, Field(ge=0, le=5)]


class _ReadyEvaluationPolicyArtifactSetV1(_Projection):
    ...
    parameters: _ArtifactSetV1Parameters


class _ReadyEvaluationPolicyArtifactSetV2(_Projection):
    status: Literal["ready"]
    criterion_id: Annotated[str, Field(min_length=1, max_length=255)]
    evaluator: Literal["artifact-set.v2"]
    configuration_digest: Annotated[
        str,
        Field(pattern=r"^sha256:[a-f0-9]{64}$"),
    ]
    parameters: _ArtifactSetV2Parameters


class _ReadyEvaluationPolicyTestRun(_Projection):
    status: Literal["ready"]
    criterion_id: Annotated[str, Field(min_length=1, max_length=255)]
    evaluator: Literal["test-run.v1"]
    configuration_digest: Annotated[
        str,
        Field(pattern=r"^sha256:[a-f0-9]{64}$"),
    ]
    parameters: _TestRunParameters


class _ReadyEvaluationPolicyBuildArtifact(_Projection):
    status: Literal["ready"]
    criterion_id: Annotated[str, Field(min_length=1, max_length=255)]
    evaluator: Literal["build-artifact.v1"]
    configuration_digest: Annotated[
        str,
        Field(pattern=r"^sha256:[a-f0-9]{64}$"),
    ]
    parameters: _BuildArtifactParameters


class _ReadyEvaluationPolicySecurityScan(_Projection):
    status: Literal["ready"]
    criterion_id: Annotated[str, Field(min_length=1, max_length=255)]
    evaluator: Literal["security-scan.v1"]
    configuration_digest: Annotated[
        str,
        Field(pattern=r"^sha256:[a-f0-9]{64}$"),
    ]
    parameters: _SecurityScanParameters


_ReadyEvaluationPolicy = Annotated[
    _ReadyEvaluationPolicyArtifactSetV1
    | _ReadyEvaluationPolicyArtifactSetV2
    | _ReadyEvaluationPolicyTestRun
    | _ReadyEvaluationPolicyBuildArtifact
    | _ReadyEvaluationPolicySecurityScan,
...
class _VerificationContext(_Projection):
    ...
    def validate_artifact_scope(self) -> _VerificationContext:
        ...
        if policy.status == "ready" and policy.criterion_id not in criterion_ids:
            raise ValueError("evaluation policy criterion is absent from Contract")
        if policy.status == "inconclusive" and not set(policy.criterion_ids) <= criterion_ids:
...
class ControlledVerifier:
    ...
    async def discover_and_verify(self, workspace_id: str) -> VerifierPollResult:
        ...
        plan = _build_plan_from_policy(policy)
...
def _build_plan_from_policy(policy: _ReadyEvaluationPolicy) -> object:
    """Build the typed evaluation plan from a discriminator-parsed policy.

    Each verifier client-side projection carries evaluator-specific
    parameters; this helper dispatches to the correct plan dataclass so the
    shared :class:`StrictVerificationEvaluator` can run it.
    """
    ...
    if evaluator == "artifact-set.v1":
        return ArtifactSetEvaluationPlan(
            **common,
    ...
    if evaluator == "artifact-set.v2":
        return ArtifactSetEvaluationPlan(
            **common,
            ...
            optional_artifact_kinds=tuple(
                kind.value for kind in policy.parameters.optional_artifact_kinds
            ),
            max_artifact_size_bytes=policy.parameters.max_artifact_size_bytes,
        )
    if evaluator == "test-run.v1":
        return TestRunEvaluationPlan(
            **common,
            minimum_test_results=policy.parameters.minimum_test_results,
            minimum_pass_rate=policy.parameters.minimum_pass_rate,
        )
    if evaluator == "build-artifact.v1":
        return BuildArtifactEvaluationPlan(
            **common,
            minimum_build_artifacts=policy.parameters.minimum_build_artifacts,
            max_artifact_size_bytes=policy.parameters.max_artifact_size_bytes,
        )
    if evaluator == "security-scan.v1":
        return SecurityScanEvaluationPlan(
            **common,
            minimum_scan_reports=policy.parameters.minimum_scan_reports,
            max_severity=policy.parameters.max_severity,
        )
    raise VerifierProtocolError(f"unsupported evaluator: {evaluator}")
